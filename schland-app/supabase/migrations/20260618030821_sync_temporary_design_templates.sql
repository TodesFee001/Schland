...
grant select, insert, update, delete on table public.temporary_design_settings to authenticated, service_role;
grant select, insert, update, delete on table public.temporary_design_templates to authenticated, service_role;
...
drop policy if exists "temporary design settings insert" on public.temporary_design_settings;
drop policy if exists "temporary design settings update" on public.temporary_design_settings;
drop policy if exists "temporary design settings delete" on public.temporary_design_settings;
...
create policy "temporary design settings insert"
on public.temporary_design_settings for insert
to authenticated
with check (
  public.has_mfa_level2()
  and (
    public.has_permission('design.manage')
    or public.has_permission('roles.manage')
    or public.has_permission('users.manage')
  )
);

create policy "temporary design settings update"
on public.temporary_design_settings for update
to authenticated
using (
  public.has_mfa_level2()
  and (
...
create policy "temporary design settings delete"
on public.temporary_design_settings for delete
to authenticated
using (
  public.has_mfa_level2()
  and (
...
drop policy if exists "temporary design templates insert" on public.temporary_design_templates;
drop policy if exists "temporary design templates update" on public.temporary_design_templates;
drop policy if exists "temporary design templates delete" on public.temporary_design_templates;
...
create policy "temporary design templates insert"
on public.temporary_design_templates for insert
to authenticated
with check (
  public.has_mfa_level2()
  and (
...
create policy "temporary design templates update"
on public.temporary_design_templates for update
to authenticated
using (
  public.has_mfa_level2()
  and (
...
create policy "temporary design templates delete"
on public.temporary_design_templates for delete
to authenticated
using (
  public.has_mfa_level2()
  and (
...
values
  ...
  ('wm-2026', 'WM 2026', 'Fussball-Weltmeisterschaft 2026', true, false, false, '2026-06-11', '2026-07-19', null, 0, 0, 90, '{"accentColor":"#111111","accentSoftColor":"#fff1b8","accentStrongColor":"#d90429","backgroundClass":"theme-wm-2026","backgroundColor":"#eadfbd","bannerEnabled":true,"bannerLabel":"FIFA WM 2026","buttonColor":"#111111","decoration":"pitch","headerStyle":"stadium"}'::jsonb),
